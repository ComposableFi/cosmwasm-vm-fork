...
import init, {
  ...
  InitOutput,
  vm_continue_execute,
  vm_continue_instantiate,
  vm_continue_migrate,
  vm_execute,
  vm_instantiate,
  vm_migrate,
  vm_query,
} from './pkg/typescript_bindings.js';
...
export type VMHost = {
  ...
  addr_canonicalize: (
    addr: string
  ) => Result<Result<Array<number>, Error>, Error>;
  ...
  db_scan: (
    ...
    order: Order
  ) => Result<StorageIterator, Error>;
  db_next: (
    iterator_id: StorageIterator
  ) => Result<[StorageKey, StorageValue], Error>;

  // Execution
  query_continuation: (
    address: string,
    message: Array<number>
  ) => Result<Result<Binary, Error>, Error>;
  continue_instantiate: (
    metadata: ContractMeta,
    funds: Array<Coin>,
    message: Array<number>,
    event_handler: any
  ) => Result<[string, Option<Binary>], Error>;
  continue_execute: (
    address: string,
    funds: Array<Coin>,
    message: Array<number>,
  ...
  query_raw: (
    address: string,
    key: StorageKey
  ) => Result<Option<Array<number>>, Error>;
...
): Promise<InitOutput> => init(module_or_path);

export const vmInstantiate = <T>(
  host: VMHost,
  code: Uint8Array,
  message: T
): Result<VMStep, Error> => vm_instantiate(host, code, JSON.stringify(message));

export const vmExecute = <T>(
  host: VMHost,
  code: Uint8Array,
  message: T
): Result<VMStep, Error> => vm_execute(host, code, JSON.stringify(message));

export const vmMigrate = <T>(
  host: VMHost,
  code: Uint8Array,
  message: T
): Result<VMStep, Error> => vm_migrate(host, code, JSON.stringify(message));

export const vmContinueInstantiate = <T>(
  host: VMHost,
  code: Uint8Array,
  message: T,
  event_handler: any
...
export const vmContinueExecute = <T>(
  host: VMHost,
  code: Uint8Array,
  message: T,
  event_handler: any
...
export const vmContinueMigrate = <T>(
  host: VMHost,
  code: Uint8Array,
  message: T,
  event_handler: any
...
export const vmQuery = <T>(
  host: VMHost,
  code: Uint8Array,
  query: T
): Result<Result<Result<Binary, Error>, Error>, Error> =>
